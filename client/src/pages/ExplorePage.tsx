...
export default function ExplorePage() {
  ...
  const apiProviders = trpc.providers.list.useQuery({
    category: state.category !== 'all' ? state.category : undefined,
    city: state.region !== 'all' ? state.region : undefined,
    minRating: state.rating !== 'all' ? Number(state.rating) / 20 : undefined,
    verifiedOnly: state.verified === 'verified',
  })

  // 过滤 + 排序（API 优先，演示数据作为未初始化数据库时的回退）
  const result = useMemo(() => {
    const filtered = filterProviders(PROVIDERS, state, CATEGORY_LABELS)
    const localResult = state.verified === 'verified' ? filtered.filter((provider) => provider.badges.includes('platform')) : filtered
    return apiProviders.data?.length ? localResult.filter((provider) => apiProviders.data?.some((row) => row.displayName === provider.nickname)) : localResult
  }, [state, searchParams, apiProviders.data])
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-full border border-border bg-card px-3 py-2 text-sm text-foreground shadow-sm">
        <input type="checkbox" checked={state.verified === 'verified'} onChange={(event) => onChange({ verified: event.target.checked ? 'verified' : 'all' })} className="size-4 accent-primary" />
        仅看平台认证服务商
      </label>
